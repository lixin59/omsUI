...
export default function ImportAssetsFile(props: tProps) {
  const { onBeforeUpload, onUploadComplete, showUploadProgress, importFileApi } = props;
  ...
  const uploadFile = async () => {
    try {
      ...
      const res = await importFileApi({ files: fileList });
      if (res.code !== '200') {
        enqueueSnackbar(`导入文件失败：${res.msg}`, {
          autoHideDuration: 5000,
          variant: 'error'
        });
        return;
      }
      enqueueSnackbar(`导入文件成功：${res.msg}`, {
        autoHideDuration: 5000,
        variant: 'success'
      });
